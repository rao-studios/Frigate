...
Frigate's `VisionAX` module (vended by the **`FrigateVision`** product) is the
...
- **`VisionAXCore`** — the AX/A11Y data structures (`AXNodeSnapshot`, `AXWindowSnapshot`,
  ...
  vocabulary. This module re-exports it, so `import VisionAX` carries those types.
...
| `Sources/VisionAX` | The Swift face: `VisionEngine` (`perceive` → `VisionScene`), `RegionClassifier`, the page map, the media lane, text recognition. |
| `Sources/VisionAX/Resources/Models` | The bundled classifier (git-lfs), exported by VisionAX's training pipeline. |
| `Tests/VisionAXTests` | The engine's tests: synthetic screens, the tiny arithmetic classifier, real captures. |
...
`Tests/VisionAXTests/Fixtures/media`. M4 Max, debug build, median of seven, nothing else
...
```swift
import VisionAX   // the runtime, plus VisionAXCore's AX types
...
and target — **`Frigate_VisionAX.bundle`** — so a copied `.app` must carry it in
