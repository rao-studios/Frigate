...
`uv run vxtrain export --out ../../Frigate/Sources/VisionAX/Resources/Models`, run from
`VisionAX/Training`.
...
model by depending on Frigate's `FrigateVision` product — but an app bundle must copy
`Frigate_VisionAX.bundle` into `Contents/Resources` for `Bundle.module` to find it.
