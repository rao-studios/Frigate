...
public class BertModel: Module, EmbeddingModel {
    ...
    private static let weightKeyRewrites: [(String, String)] = [
        (".layer.", ".layers."),
        (".self.key.", ".key_proj."),
        ...
        ("cls.predictions.transform.LayerNorm.", "lm_head.ln."),
        ("cls.predictions.decoder", "lm_head.decoder"),
        ("cls.predictions.transform.norm.weight", "lm_head.ln.weight"),
        ("cls.predictions.transform.norm.bias", "lm_head.ln.bias"),
        ("cls.predictions.bias", "lm_head.decoder.bias"),
        ("bert.", ""),
    ]

    public func sanitize(weights: [String: MLXArray]) -> [String: MLXArray] {
        // Table-driven rather than one 18-link `.replacingOccurrences` chain: on Linux the
        // type checker gives up on that expression ("unable to type-check in reasonable
        // time"). Order is significant and preserved exactly — the specific LayerNorm and
        // cls.predictions rules must run before the general ones below them.
        weights.reduce(into: [:]) { result, item in
            var key = item.key
            for (find, replace) in Self.weightKeyRewrites {
                key = key.replacingOccurrences(of: find, with: replace)
            }
